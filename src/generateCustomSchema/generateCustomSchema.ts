...
export async function generateCustomSchema (application_context: string | null, table_definitions: string): Promise<string> {
    ...
    const completion = await openai.chat.completions.create({
       messages:[
        ...
        {
            role: "user",
            content: `Generate a PostgreSQL schema based on the following user specifications.
                The schema should include all necessary tables and fields as described,
                with proper SQL syntax for creating tables. 
                Ensure that primary keys and foreign keys are correctly defined based on the user inputs.
                Additionally, consider any provided context about the application type to optimize the schema's 
            ...
            Please format the SQL schema with proper data types, constraints, and relationships.
